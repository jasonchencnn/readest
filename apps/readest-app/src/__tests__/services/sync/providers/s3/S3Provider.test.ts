...
describe('S3Provider — transport', () => {
  ...
  test('sends keys with sub-delims in SigV4 canonical (RFC 3986) form', async () => {
    ...
    await h.provider.head("/Moyue/books/h1/测试(test)*!'.epub");
    const objectUrl = h.url(0);
    expect(new URL(objectUrl).pathname).toBe(
      '/readest/Moyue/books/h1/%E6%B5%8B%E8%AF%95%28test%29%2A%21%27.epub',
    );
    ...
    await h.provider.list('/Moyue/(dir)');
    const listUrl = h.url(1);
    expect(new URL(listUrl).search).toContain('prefix=Moyue%2F%28dir%29%2F');
  ...
  test('deleteDir lists the prefix and DELETEs every key', async () => {
    ...
    h.fetchMock
      .mockResolvedValueOnce(
        xml(
          listPage({
            keys: [{ key: 'Moyue/books/h1/config.json' }, { key: 'Moyue/books/h1/B (1).epub' }],
          }),
        ),
      )
    ...
    expect(new URL(h.url(2)).pathname).toBe('/readest/Moyue/books/h1/B%20%281%29.epub');
  });
