...
describe('microsoftOAuthConfig', () => {
  test('builds the Microsoft OAuth config with the custom-scheme redirect', () => {
    ...
    expect(cfg.redirectUri).toBe('moyue-onedrive://auth');
    expect(cfg.redirectScheme).toBe('moyue-onedrive');
    expect(cfg.authParams).toEqual({ prompt: 'select_account' });
  });

  test('isOneDriveOAuthRedirectUrl flags the OneDrive redirect scheme', () => {
    expect(isOneDriveOAuthRedirectUrl('moyue-onedrive://auth?code=x&state=y')).toBe(true);
    expect(isOneDriveOAuthRedirectUrl('moyue://auth-callback')).toBe(false);
