...
describe('parseShareDeepLink', () => {
  ...
  it('parses moyue://share/{token}', () => {
    expect(parseShareDeepLink(`moyue://share/${VALID_TOKEN}`)).toEqual({ token: VALID_TOKEN });
  });
  ...
  it('rejects tokens of the wrong length', () => {
    expect(parseShareDeepLink('moyue://share/short')).toBeNull();
    expect(parseShareDeepLink(`moyue://share/${VALID_TOKEN}extra`)).toBeNull();
  });
  ...
  it('rejects tokens with disallowed characters', () => {
    ...
    expect(parseShareDeepLink(`moyue://share/${bad}`)).toBeNull();
  });
  ...
  it('rejects moyue:// URLs whose host is not "share"', () => {
    expect(parseShareDeepLink(`moyue://book/${VALID_TOKEN}`)).toBeNull();
    expect(parseShareDeepLink(`moyue://annotation/${VALID_TOKEN}`)).toBeNull();
  });
