...
const URL_A = 'moyue://book/hashA';
const URL_B = 'moyue://book/hashB';
...
describe('markLaunchUrl', () => {
  ...
  it('bounds the per-scope set', async () => {
    const { markLaunchUrl } = await loadModule('run-1');
    for (let i = 0; i < 40; i++) markLaunchUrl('book', `moyue://book/h${i}`);
    ...
    expect(stored.urls[0]).toBe('moyue://book/h24');
    expect(markLaunchUrl('book', 'moyue://book/h39')).toBe(false);
  });
