...
export interface IngestFileDeps {
  ...
  isLoggedIn: boolean;
  /**
   * Pre-resolved absolute path to Moyue's own `Books/` directory. When
...
export interface IngestFileOptions {
  ...
  groupId?: string;
  groupName?: string;
  /** Tag parsed from a Send-to-Moyue email subject (`#scifi`). */
  subjectTag?: string;
 ...
 *     explicitly told Moyue to read in place. The Moyue data location
 *     (`customRootDir`) is intentionally NOT an in-place trigger; that
 *     directory is Moyue's own home and may freely contain hash copies.
 *   - The path is NOT inside Moyue's own managed books directory
 *     (`appBooksPrefix`, e.g. `<AppData>/Books/`). Anything in that subtree
 *     is a hash copy under Moyue's control, no point marking it in-place.
...
function shouldImportInPlace(
  ...
  // If the file already lives inside Moyue's own managed books directory
...
export async function ingestFile(
  ...
  // Moyue Cloud storage is written only when Moyue Cloud is one of the
