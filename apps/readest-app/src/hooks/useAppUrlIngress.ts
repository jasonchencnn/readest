...
interface SharedIntentPayload {
  urls: string[];
  /**
   * Android-only. Distinguishes "Open with Moyue" (`VIEW` — the user
   * tapped a file in their file browser and chose Moyue) from "Send to
   * Moyue" (`SEND` / `SEND_MULTIPLE` — share-sheet capture). We forward
 ...
 *   - `shared-intent` plugin   — Android "Share to Moyue" intent
...
export function useAppUrlIngress() {
  ...
  useEffect(() => {
    ...
    // side, so any "Open with Moyue" intent that arrived AFTER cold
